...
lookup_results <- name_backbone_checklist(name_data) %>%
  ...
  # select(1:11) %>%                    # Select only essential taxonomic information (w/o phylum, family)
  select(1:13) %>%                    # Select only essential taxonomic information
...
df.merged <- df.fullredlist %>% # get raw list
  left_join(lookup_results, by = join_by(verbatimScientificName)) %>% # join with normalized (matched) data
  # Following steps are technical:
  relocate(ID) %>% # relocate ID variable to the 1st place
  relocate(usageKey, .after = ID) %>% # relocate key variable next to IDs
  ...
  rename(phylum = phylum.x) %>%    # rename phylum
  relocate(family, .after = class) # relocate family after other taxonomy fields



# Splitting full normalized data by matchType ####
# Names whose matching is satisfactory - we will then look for occurrences for 
# these names using taxonKey field.
goodmatch <- df.merged %>% 
...
# Save outputs and close the session ####
